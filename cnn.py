...
__all__ = ['NonBottleneck1D', 'ResNet', 'ResNet18', 'ResNet34', 'ResNet50', 'ResNet101', 'ResNet152']
...
def ResNet18(pretrained_on_imagenet=False,
    ...
    if kwargs['block'] != BasicBlock and pretrained_on_imagenet:
        ...
        state_dict = weights.copy()
        if 'input_channels' in kwargs and kwargs['input_channels'] == 1:
            # sum the weights of the first convolution
            state_dict['conv1.weight'] = torch.sum(weights['conv1.weight'],
                                                axis=1, keepdim=True)
...
def ResNet101(pretrained_on_imagenet=False, **kwargs):
    model = ResNet([3, 4, 23, 3], Bottleneck, **kwargs)
    if 'input_channels' in kwargs and kwargs['input_channels'] == 1:
        input_channels = 1
    else:
        input_channels = 3
    if pretrained_on_imagenet:
        weights = model_zoo.load_url(model_urls['resnet101'], model_dir='./')
        if input_channels == 1:
            # sum the weights of the first convolution
            weights['conv1.weight'] = torch.sum(weights['conv1.weight'],
                                                axis=1, keepdim=True)
        ...
        weights.pop('fc.bias')
        model.load_state_dict(weights, strict=False)
        print('Loaded ResNet101 pretrained on ImageNet')
    return model


def ResNet152(pretrained_on_imagenet=False, **kwargs):
    model = ResNet([3, 8, 36, 3], Bottleneck, **kwargs)
    if 'input_channels' in kwargs and kwargs['input_channels'] == 1:
        input_channels = 1
    else:
        input_channels = 3
    if pretrained_on_imagenet:
        weights = model_zoo.load_url(model_urls['resnet152'], model_dir='./')
        if input_channels == 1:
            # sum the weights of the first convolution
            weights['conv1.weight'] = torch.sum(weights['conv1.weight'],
                                                axis=1, keepdim=True)
        ...
        weights.pop('fc.bias')
        model.load_state_dict(weights, strict=False)
        print('Loaded ResNet152 pretrained on ImageNet')
    return model
...
if __name__ == '__main__':
    model = ResNet18(block='NonBottleneck1D', pretrained_on_imagenet=True,dilation=[1]*4)
