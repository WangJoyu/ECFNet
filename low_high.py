...
class AFNB(nn.Module):
    ...
    def __init__(self, low_in_channels, high_in_channels, out_channels, key_channels, value_channels, dropout,
                 sizes=([1]), norm_type=None, psp_size=(1,3,6,8,12)):
        ...
        self.cb1x1 = nn.Sequential(
            nn.Conv2d(2*high_in_channels, high_in_channels, kernel_size=1),
            nn.BatchNorm2d(high_in_channels),
            nn.ReLU()
        )
    ...
    def forward(self, low_feats, high_feat1, high_feat2):
        high_feats = torch.cat([high_feat1, high_feat2], dim=1)
        high_feats = self.cb1x1(high_feats)
        # high_feats = high_feat1+high_feat2
...
class AFNB2(nn.Module):
    ...
    def __init__(self, low_in_channels, high_in_channels, out_channels, key_channels, value_channels, dropout,
                 sizes=([1]), norm_type=None, psp_size=(1,3,6,8,12,15)):
        ...
        self.cb1x1_last = nn.Sequential(
            nn.Conv2d(2*low_in_channels, low_in_channels, kernel_size=1),
    ...
    def forward(self, low_feats=None, high_feat1=None, high_feat2=None):
        high_feats = torch.cat([high_feat1, high_feat2], dim=1)
        high_feats = self.cb1x1(high_feats)
        if low_feats is None:
            low_feats = high_feats
        elif isinstance(low_feats, list):
            low_feats = self.cb1x1_last(torch.cat(low_feats, dim=1))
        # high_feats = high_feat1+high_feat2
        priors = [stage(low_feats, high_feats) for stage in self.stages]
        context = priors[0]
        for i in range(1, len(priors)):
