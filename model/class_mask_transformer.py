...
class Transformer(nn.Module):
    ...
    def __init__(self, embed_dims=[64, 128, 256, 512], num_heads=[1, 2, 4, 8], mlp_ratios=[4, 4, 4, 4], qkv_bias=False,
                 ...
                 num_decoder_layers=[1, 1, 1, 1], grid_sizes=[8, 4, 2, 1], num_class=9):
        super().__init__()
        ...
        self.decoder = TransformerDecoder(hidden_dim, [8, 8, 8, 8], mlp_ratios[::-1], num_decoder_layers[::-1],
    ...
    def forward(self, features, query_embed, pos_embed=None):
        ...
        outputs, masks = self.decoder(memory, query_embed)
        outputs = [F.interpolate(output, (H, W), align_corners=True, mode='bilinear') for output in outputs]
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, features, pos=None, mask=None):
        ...
        for feature, linear in zip(features, self.linear):
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, mlp_ratios, num_decoder_layers,
                 norm_layer, grid_sizes=1, sr_ratio=1, num_class=9):
        super().__init__()
        self.layers = nn.ModuleList()
        for i, num in enumerate(num_decoder_layers):
            ...
            layer = [DecoderLayer(embed_dims[i], num_heads[i], mlp_ratios[i],
                     ...
                     for _ in range(num)]
            self.layers.append(nn.Sequential(*layer))'''
            layer = DecoderLayer(embed_dim, num_heads[i], mlp_ratios[i], norm_layer=norm_layer, num_class=num_class)
            self.layers.append(layer)
    ...
    def forward(self, features, query_pos=None, pos=None):
        ...
        for feature, layer in zip(features, self.layers):
            # B, C, H, W = feature.shape
            # feature = feature.reshape(B, C, -1).permute(0, 2, 1)
            # cls_tokens, mask = layer_mhsa(feature, cls_tokens)
            # output = feature + layer_mha(feature, cls_tokens, feature_size=(H, W))
            output, cls_tokens, mask = layer(cls_tokens, feature)
...
class DecoderLayer(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                act_layer=nn.GELU, norm_layer=nn.LayerNorm, sr_ratio=1, num_class=9):
        super().__init__()
        '''
        self.norm1 = norm_layer(dim)
        self.self_attn = Attention(
            ...
            num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
            attn_drop=attn_drop, proj_drop=drop, sr_ratio=sr_ratio)'''
        self.mha = Attention(dim, num_heads, sr_ratio=sr_ratio)
        self.mhsa = Attention(dim, num_heads, sr_ratio=sr_ratio)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm1 = norm_layer(dim)
        self.norm2 = norm_layer(dim)
        ...
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
        self.mask_generator = nn.Conv2d(dim, num_class, 3, 1, 1)
        self.alignment = nn.Conv2d(dim, dim, 3, 1, 1)
    ...
    def forward(self, cls_tokens, feature):
        B, C, H, W = feature.shape
        feature_size = [H, W]
        align = self.alignment(feature).reshape(B, C, -1).transpose(-1, -2)
        mask = self.mask_generator(feature)
        mask_softmax = F.softmax(mask.reshape(B, -1, H*W), dim=-1)
        x = mask_softmax @ align
        #cls_tokens = x + self.drop_path(self.mhsa(q=x, kv=cls_tokens, feature_size=[120, 160]))
        cls_tokens = cls_tokens + self.drop_path(self.mhsa(q=cls_tokens, kv=x, feature_size=[120, 160]))
        cls_tokens = self.norm1(cls_tokens)
        feature_tokens = feature.reshape(B, C, -1).permute(0, 2, 1)
        feature_tokens = feature_tokens + self.drop_path(self.mha(q=feature_tokens, kv=cls_tokens, feature_size=feature_size))
        feature_tokens = self.norm2(feature_tokens)
        feature_tokens = feature_tokens + self.drop_path(self.mlp(feature_tokens, feature_size=feature_size))
        feature = self.norm3(feature_tokens).permute(0, 2, 1).reshape(B, C, H, W)

        return feature, cls_tokens, mask
