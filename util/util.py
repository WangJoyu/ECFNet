...
def downsample(labels, down_rate=2):
    B, H, W = labels.shape
    assert H % down_rate == 0, f'please make sure height of image % {down_rate} = 0'
    assert W % down_rate == 0, 'please make sure width of image % 2 = 0'
    out = torch.zeros((B, H//down_rate, W//down_rate), dtype=labels.dtype, device=labels.device)
    for i in range(H//down_rate):
        for j in range(W//down_rate):
            h, w = i*down_rate, j*down_rate
            out[:, i, j] = torch.mode(labels[:, h:h+down_rate, w:w+down_rate].reshape(B, -1), dim=-1)[0]
    return out
    
    
def cosine_similarity_loss(predict, label):
    predict = F.sigmoid(predict)
    loss = 1-torch.mean(F.cosine_similarity(predict, label, dim=1))
    return loss
